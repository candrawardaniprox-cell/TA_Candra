"""
augment_geometry_clahe_median.py

Script augmentasi dataset offline yang FOKUS PADA GEOMETRI, CLAHE, & MEDIAN FILTER.

Sesuai permintaan:
1. SEMUA gambar di dataset (baik data asli maupun augmentasi) akan dikenakan
   2 teknik enhancement secara berurutan:
   a. CLAHE  - untuk meningkatkan kontras secara adaptif dan merata.
   b. Median Filter - untuk mengurangi noise (terutama salt-and-pepper noise)
      tanpa mengaburkan tepi objek secara berlebihan.
...
Output:
  data/Dataset2026_clahe_median_geo/

Cara pakai:
  python augment_geometry_clahe_median.py --overwrite
"""
...
def apply_median_filter(image: np.ndarray, ksize: int = 3) -> np.ndarray:
    ...
    Parameter:
        ...
        ksize  : Ukuran kernel median filter (harus bilangan ganjil: 3, 5, 7, ...).
                 Default = 3 (kernel 3x3). Semakin besar nilainya, semakin kuat
                 efek pengurangan noise, tetapi detail halus bisa ikut hilang.
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--output-dir", type=Path, default=Path("data/Dataset2026_clahe_median_geo"))
...
def main():
    ...
    print("=====================================================")
    print("  CLAHE + MEDIAN FILTER + GEO AUGMENTATION PIPELINE")
    print("=====================================================")
    
    # ---- 1. Proses Data Asli dengan CLAHE + Median Filter ----
    print("\n[1/3] Menerapkan CLAHE + Median Filter ke seluruh dataset asli...")
    copied = 0
    for img_info in coco["images"]:
        ...
        if src.exists():
            ...
            if img is not None:
                ...
                copied += 1
    print(f"  [OK] {copied} gambar asli telah diproses CLAHE + Median Filter.")
    ...
    while attempts < max_attempts:
        ...
        # Baca citra yang SUDAH di-enhance (CLAHE + Median Filter) dari output_dir
        # Hal ini menjamin citra asli dan augmentasi punya basis enhancement yang persis sama
    ...
    print("\n=====================================================")
    print("  HASIL AKHIR AUGMENTASI GEOMETRI + CLAHE + MEDIAN FILTER")
    print("=====================================================")
