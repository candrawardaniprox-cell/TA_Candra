"""
Offline COCO train augmentation, Roboflow-like.

Script ini berdiri sendiri dan tidak mengubah pipeline training utama.
Default input mengikuti Config:
  data/coco copy/train2017
  data/coco copy/annotations_coco/instances_train2017.json

Default output menjaga nama folder/file COCO yang sama:
  data/coco copy_train_weighted_default/train2017
  data/coco copy_train_weighted_default/annotations_coco/instances_train2017.json

Contoh:
  python offline_augment_train_roboflow.py
  python offline_augment_train_roboflow.py --multiplier 3
  python offline_augment_train_roboflow.py --multiplier 2
  python offline_augment_train_roboflow.py --balance-to 600
  python offline_augment_train_roboflow.py --class-multipliers "moler=1.17,slabung=0.01,ulat_grayak=0.06" --include-original
  python offline_augment_train_roboflow.py --multiplier 3 --include-original
  python offline_augment_train_roboflow.py --multiplier 3 --seed 123 --overwrite
...
# target total train = data asli + augmentasi tambahan.
# moler       augmentasi tambahan x1.309
# slabung     augmentasi tambahan x1
# ulat_grayak augmentasi tambahan x1.343
DEFAULT_TRAIN_EXTRA_CLASS_MULTIPLIERS = {
    "moler": 0.34,
    "slabung": 0.0,
    "ulat_grayak": 0.38, 
}
DEFAULT_TRAIN_CLASS_MULTIPLIERS_ARG = ",".join(
    f"{class_name}={value}"
    for class_name, value in DEFAULT_TRAIN_EXTRA_CLASS_MULTIPLIERS.items()
)
...
def default_paths() -> Tuple[Path, Path, Path]:
    try:
        ...
        image_dir = Path(Config.TRAIN_IMAGES)
        annotation_file = Path(Config.TRAIN_ANNOTATIONS)
    except Exception:
        data_root = Path("data") / "coco copy"
        image_dir = data_root / "train2017"
        annotation_file = data_root / "annotations_coco" / "instances_train2017.json"
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Buat dataset train COCO teraugmentasi secara offline."
    )
    parser.add_argument(
        "--input-images",
        type=Path,
        default=image_dir,
        help=f"Folder gambar train. Default: {image_dir}",
    )
    parser.add_argument(
        "--input-json",
        type=Path,
        default=annotation_file,
        help=f"File JSON COCO train. Default: {annotation_file}",
    )
    parser.add_argument(
        "--output-root",
        type=Path,
        ...
        help=(
            ...
            "<data_root>_train_weighted_default, "
            "<data_root>_train_augmented_<multiplier>x, atau <data_root>_train_balanced_<target>"
        ),
    )
    parser.add_argument(
        "--multiplier",
    ...
    parser.add_argument(
        "--class-multipliers",
        type=str,
        default=DEFAULT_TRAIN_CLASS_MULTIPLIERS_ARG,
        help=(
            ...
            f"Default: \"{DEFAULT_TRAIN_CLASS_MULTIPLIERS_ARG}\""
        ),
    )
    parser.add_argument(
        "--balance-to",
    ...
    if args.output_root is None:
        if args.balance_to is not None:
            args.output_root = data_root.parent / f"{data_root.name}_train_balanced_{args.balance_to}"
        elif args.balance_targets is not None:
            args.output_root = data_root.parent / f"{data_root.name}_train_balanced_custom"
        elif args.class_multipliers == DEFAULT_TRAIN_CLASS_MULTIPLIERS_ARG and args.include_original:
            args.output_root = data_root.parent / f"{data_root.name}_train_weighted_default"
        elif args.class_multipliers:
            args.output_root = data_root.parent / f"{data_root.name}_train_weighted_custom"
        else:
            args.output_root = data_root.parent / f"{data_root.name}_train_augmented_{args.multiplier}x"
...
def ensure_output_dirs(output_root: Path, overwrite: bool) -> Tuple[Path, Path]:
    ...
    image_out_dir = output_root / "train2017"
...
def write_output_json(new_coco: Dict, annotation_out_dir: Path) -> Path:
    output_json = annotation_out_dir / "instances_train2017.json"
...
def run_multiplier_mode(
    *,
    ...
    new_coco = build_output_coco(coco, f"offline train augmentation {args.multiplier}x")
...
def run_balance_mode(
    *,
    ...
    new_coco = build_output_coco(coco, "offline train class-balanced augmentation")
    next_image_id, next_ann_id = next_ids(coco)
    missing_images = 0
    skipped_empty_aug = 0
    source_aug_counts: Counter = Counter()
