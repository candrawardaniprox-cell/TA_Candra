...
class DynamicCNNBackbone(nn.Module):
    def __init__(self, backbone_name='resnet18', pretrained=True, pretrain_source='imagenet', custom_weights_path=None):
        ...
        # ============ Mekanisme Freezing Backbone ============
        # Jika BACKBONE_FREEZE=True, kunci semua parameter backbone
        # agar tidak ikut di-update oleh optimizer saat training.
        # Referensi: Yosinski et al. (NeurIPS 2014), Faster R-CNN (Ren et al., 2015)
        from config import Config
        self._is_frozen = bool(getattr(Config, 'BACKBONE_FREEZE', False))
        if self._is_frozen and self.backbone_name not in ('none',):
