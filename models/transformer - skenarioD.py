...
# =============================================================================
# =============================================================================
# CPSA — Standard Self-Attention (Skenario C - Ablation)
# =============================================================================
# =============================================================================
# CPSA — Standard Self-Attention (Skenario C - Ablation)
# =============================================================================
...
class CPSA(nn.Module):
    """
    Standard Self-Attention (Modifikasi Skenario C).
    
    Menghapus Convolutional Spatial Reduction (sr_conv & sr_pw).
    Q, K, dan V diproyeksikan langsung dari input 'x' murni seperti pada ViT asli.
    """
    ...
    def __init__(self, dim: int, num_heads: int = 4, reduction_ratio: int = 2):
        ...
        # KV projection (memproyeksikan dari x asli, bukan dari S)
    ...
    def forward(self, x: torch.Tensor, H: int, W: int) -> torch.Tensor:
        ...
        # K dan V diambil langsung dari 'x' (tanpa disaring CNN)
        kv = self.kv_proj(x)
        kv = kv.reshape(B, N, 2, self.num_heads, self.dim_head)
        kv = kv.permute(2, 0, 3, 1, 4)
        k, v = kv[0], kv[1]

        # --- SOLUSI ANTI-MELEDAK UNTUK FP16 ---
        # Kita hitung Attention murni secara paksa di presisi tinggi (Float32)
        # agar perkalian q dan k tidak melebihi batas maksimal (overflow).
        q_fp32 = q.to(torch.float32)
        k_fp32 = k.to(torch.float32)
        
        attn = (q_fp32 @ k_fp32.transpose(-2, -1)) * self.scale
        attn = attn.softmax(dim=-1)
        
        out  = (attn @ v.to(torch.float32)).to(q.dtype) # Kembalikan ke tipe asal
        # --------------------------------------
...
class LFFN(nn.Module):
    ...
    def forward(self, x: torch.Tensor, H: int, W: int) -> torch.Tensor:
        ...
        #SKENARIO B ---
        out = self.act(self.bn2(self.pw_expand(x_2d))) # Langsung tembak ke pw_expand
        
        # --- KODE ASLI LANJUTAN (Tidak dipagar) ---
        out = self.bn4(self.pw_reduce(out))
