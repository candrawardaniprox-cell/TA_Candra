...
Script untuk menerapkan CLAHE + Median Filter ke SEMUA gambar
...
yang telah diterapkan CLAHE + Median Filter.

Cara pakai:
  python apply_clahe_only.py --overwrite
  python apply_clahe_only.py --input "data/coco copy" --overwrite
  python apply_clahe_only.py --input "data/coco copy" --output "data/coco copy_clahe_median" --overwrite
"""
...
def apply_enhancement(image: np.ndarray, rng: random.Random) -> np.ndarray:
    """
    Pipeline enhancement lengkap:
      1. CLAHE   (peningkatan kontras adaptif)
      2. Median Filter ksize=3 (pengurangan noise halus)
    """
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Terapkan CLAHE + Median Filter ke seluruh gambar dataset split "
                    "(train/val/test). Tanpa augmentasi, hanya enhancement."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="Folder output. Default: <input>_clahe_median",
    )
    parser.add_argument(
...
def main():
    ...
    if args.output is None:
        args.output = Path(str(args.input) + "_clahe_median")

    print("=====================================================")
    print("  APPLY CLAHE + MEDIAN FILTER (TANPA AUGMENTASI)")
    print("=====================================================")
    ...
    # ---- 2. Proses Gambar dengan CLAHE + Median Filter ----
    print("\n[2/2] Menerapkan CLAHE + Median Filter ke seluruh gambar...")
    ...
    for subfolder_name in IMAGE_SUBFOLDERS:
        ...
        for img_path in image_files:
            ...
            # Terapkan CLAHE + Median Filter
            img_clahe = apply_enhancement(img, rng)

            # Simpan ke folder output dengan nama yang sama
            dst_path = dst_folder / img_path.name
            cv2.imwrite(str(dst_path), img_clahe, [cv2.IMWRITE_JPEG_QUALITY, 95])
            processed += 1

        total_processed += processed
        total_skipped += skipped
        print(f"  [OK] {subfolder_name}: {processed} gambar diproses CLAHE + Median Filter"
              + (f", {skipped} dilewati" if skipped else ""))

    # ---- Ringkasan ----
    print("\n=====================================================")
    print("  SELESAI - CLAHE + MEDIAN FILTER ENHANCEMENT")
    print("=====================================================")
